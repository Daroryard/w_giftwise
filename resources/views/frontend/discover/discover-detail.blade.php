  ...
  setTimeout(function() {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        let unique_category = [...new Map(response.category.map(item => [item['conf_category_name_th'], item])).values()];
        ...
        $.each(unique_category, function(index, value) {
          ...
          if (value.conf_category_name_en == null) {

          } else {
            category_en += `<li class="list-group-item"><a href="{{url('/product-quick-filter/` + value.conf_mainproduct_id + `')}}" style="color:black">` + value.conf_category_name_en + `</a></li>`
          }

          if (value.conf_category_name_th == null) {

          } else {
            category_th += `<li class="list-group-item"><a href="{{url('/product-quick-filter/` + value.conf_mainproduct_id + `')}}" style="color:black">` + value.conf_category_name_th + `</a></li>`
          }
        ...
        setTimeout(function() {
          $('#result-category').removeAttr('hidden');

          if (unique_category == '' && unique_productname == '') {

            $('#result-category').attr('hidden', 'hidden');

          } else {

            $('.scroll-list-category').html(
