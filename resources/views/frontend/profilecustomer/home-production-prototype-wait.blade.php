...
<div class="main-container">
    ...
    <div class="main">
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="page-title">
                            <h3 class="text-black">
                                @if (app()->getLocale() == 'th')
                                รายการสั่งผลิต: กำลังผลิตต้นแบบ ({{ $count_confirm }} รายการ)
                                @else
                                Production order list: Prototype ({{ $count_confirm }} items)                          
                                @endif
                            </h3>
                        </div>
                        <span class="product-tag link-prototype-all">
                            @if (app()->getLocale() == 'th')
                            <span class="badge badge-none">แสดงทั้งหมด</span>
                            @else
                            <span class="badge badge-none">Show all</span>
                            @endif
                        </span>
                        <span class="product-tag link-prototype-create">
                            @if (app()->getLocale() == 'th')
                            <span class="badge badge-none">กำลังสร้างต้นแบบ</span>
                            @else
                            <span class="badge badge-none">Creating prototype</span>
                            @endif
                        </span>
                        <span class="product-tag link-prototype-wait">
                            @if (app()->getLocale() == 'th')
                            <span class="badge badge-active">รอยืนยันต้นแบบ</span>
                            @else
                            <span class="badge badge-active">Waiting for prototype confirmation</span>
                            @endif
                        </span>
                        <span class="product-tag link-prototype-edit">
                            @if (app()->getLocale() == 'th')
                            <span class="badge badge-none">รอแก้ไข</span>
                            @else
                            <span class="badge badge-none">Waiting for edit</span>
                            @endif
                        </span>
                    </div>                     
                    <div class="col-sm-6">
        ...
        <div class="product-wrapper-grid">
            ...
            @foreach($production_prototype as $product)
...
<!--modal edit modal start-->
<div class="modal fade bd-example-modal theme-modal" id="editModal" tabindex="-1" role="dialog" aria-hidden="true">
            ...
            <div class="row">
              <div class="col-12">
                <div class="modal-bg">
                  ...
                  <div class="hs-toast-wrapper  hs-toast-fixed-top ">
                    <h4>แก้ไขเพิ่มเติม</h4>
                    <textarea class="form-control" id="remarkedit" rows="5"  style="border-radius: 10px;" placeholder="กรุณาระบบรายละเอียดที่ต้องการแก้ไข"></textarea>      
                     <div style="text-align: right;margin-top:10px;" id="bt_action_edit">
...
<script>
    ...
    $(document).ready(function() {
        $('.menu-cus5').removeClass('option');
        $('.menu-cus5').addClass('option1');
    });


    $('.link-prototype-all').click(function() {
        location.href = "/{{ app()->getLocale() }}/customer/production-prototype";
    });
    $('.link-prototype-create').click(function() {
        location.href = "/{{ app()->getLocale() }}/customer/production-prototype-create";
    });
    $('.link-prototype-wait').click(function() {
        location.href = "/{{ app()->getLocale() }}/customer/production-prototype-wait";
    });
    $('.link-prototype-edit').click(function() {
        location.href = "/{{ app()->getLocale() }}/customer/production-prototype-edit";
    });
    ...
    productDetail = (ref) => {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(data[0].sd != null) {
                    file += `
                    <a href="javascript:void(0)" target="_blank" style="color: #007275;font-size: 12px;" onclick="downloadSd(${data[0].sd.sal_saleorder_hd_id})">
                        <br>
                        <i class="fa fa-file-pdf-o"></i> ${data[0].sd.sal_saleorder_hd_docuno}.pdf
                    </a>`;
                }
                ...
                $('#bt_action').html(`
                            <button type="button" class="btn btn-edit" style="border-radius: 10px;" onclick="editModal(${data[0].sal_confirmorder_dt_id})">แก้ไขเพิ่มเติม</button>
                            <button type="button" class="btn btn-solid" style="border-radius: 10px;" onclick="onFrim(${data[0].sal_confirmorder_dt_id})">ยืนยันต้นแบบ</button>
                `);
    ...
    editModal = (ref) => {
        ...
        $('#bt_action_edit').html(`
                        <button type="button" class="btn btn-edit" style="border-radius: 10px;" onclick="returnModal(${ref})">กลับ</button>
                        <button type="button" class="btn btn-solid" style="border-radius: 10px;" onclick="saveEdit(${ref})">ส่งข้อมูล</button> 
        `);
    ...
    $.ajax({
        url: "{{ url('customer/confrim-protoype') }}",
        type: 'post',
        data: {
            _token : "{{ csrf_token() }}",
            ref: ref
    ...
    saveEdit = (ref) => {
        ...
        let detail = $('#remarkedit').val();

        $.ajax({
            url: "{{ url('customer/edit-confirm-order') }}",
            type: 'post',
            data: {
                _token: "{{ csrf_token() }}",
                ref: ref,
            ...
            success: function(data) {
                if(data.status == 'success') {
                    Swal.fire({
                        ...
                        <img src="{{ asset('assets/frontend/images/profile_customer/icon-suc.png') }}" style="width: 50px;">
                        <br><br>
                        ส่งข้อมูลการแก้ไขแล้ว
                        <br>
                        <small>ข้อมูลของคุณส่งข้อมูลการแก้ไขแล้ว</small>`,
                        confirmButtonText: 'ปิด'
                    }).then((result) => {
                        if (result.isConfirmed) {
                            location.reload();
    ...
    downloadSd = (ref) => {
        window.location.href = "{{ url('customer/download-saleorder') }}/" + ref;
    }
