...
<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/v/dt/dt-1.10.12/af-2.1.2/cr-1.3.2/rr-1.1.2/sc-1.4.2/datatables.min.css"/>
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="page-title">
                            <h3 class="text-black">
                                @if (app()->getLocale() == 'th')
                                รายการจัดส่งสินค้า ({{ $count_orderlist }} รายการ)
                                @else
                                Delivery list ({{ $count_orderlist }} items)                  
                                @endif
                            </h3>
                        </div>
                        <span class="product-tag link-in">
                            @if (app()->getLocale() == 'th')
                            <span class="badge badge-none">ที่ต้องจัดส่ง</span>
                            @else
                            <span class="badge badge-none">To be delivered</span>
                            @endif
                        </span>
                        <span class="product-tag link-success">
                            @if (app()->getLocale() == 'th')
                            <span class="badge badge-active">จำส่งสำเร็จ</span>
                            @else
            ...
            <div class="row">
                <div class="col-12">
                    <table class="table table-striped table-bordered" id="example" style="width:100%">
                        <thead>
                            <tr style="border: 0px solid #fff;width:100%">
                                <th  style="border: 0px solid #fff;">รายการ</th>
                                <th  style="border: 0px solid #fff;">Order No.</th>
                                <th style="border: 0px solid #fff;">Product Name</th>
                                <th style="border: 0px solid #fff;">วันที่ส่ง</th>
                                <th  style="border: 0px solid #fff;">จัดส่ง (ชิ้น)</th>
                                <th style="border: 0px solid #fff;">ที่อยู่จัดส่ง</th>                                                                       
                            </tr>
                        </thead>
                        <tbody>
                        @foreach($orderlist as $key => $value)                     
                        <tr style="border: 0px solid #fff;" onclick="productDetail('{{ $value->deliverydocu_dt_id }}')">
                            <td scope="col" style="border: 0px solid #fff;">
                            <img src="https://erp.giftwise.co.th/{{ $value->sal_confirmorder_dt_imgpic }}"  alt="product" style="width: 100px !important;heigth: 100px !important;">
                            <td scope="col" style="border: 0px solid #fff;">{{ $value->mpd_docuno }}</td>
                            <td scope="col" style="border: 0px solid #fff;">{{ $value->ms_product_name }}</td>
                            <td scope="col" style="border: 0px solid #fff;">{{ $value->deliverydocu_hd_date }}</td>
                            <td scope="col" style="border: 0px solid #fff;">{{ $value->deliverydocu_dt_qty }} {{ $value->ms_product_unit_name }}</td>
                            <td scope="col" style="border: 0px solid #fff;">{{ $value->transport_address }}</td>                                                                     
                        </tr>                             
                        @endforeach
                        </tbody>
                    </table>               
          ...
          <div class="container-fluid p-0">
            ...
            <div class="progress-track">
                <ul id="progressbar">
                    ...
                    <li class="step0 active" id="step3" style="text-align: center;">
                    @if (app()->getLocale() == 'th')
                    ...
                    <li class="step0 active" id="step4" style="text-align: right;">
                    @if (app()->getLocale() == 'th')
                    ...
                    <li class="step0 active" id="step5" style="text-align: right;">
                    @if (app()->getLocale() == 'th')
...
<script type="text/javascript" src="https://cdn.datatables.net/v/dt/dt-1.10.12/af-2.1.2/cr-1.3.2/rr-1.1.2/sc-1.4.2/datatables.min.js"></script>
...
<script>
$('.menu-cus11').removeClass('option');
$('.menu-cus11').addClass('option1');   
$('table').DataTable({
    ...
    language: {
        search: "_INPUT_",
        searchPlaceholder: "ค้นหาจากเลข Order, รหัสสินค้า",
    }
});
    ...
    $('.link-in').click(function() {
        location.href = "/{{ app()->getLocale() }}/customer/transport";
    });
    $('.link-success').click(function() {
        location.href = "/{{ app()->getLocale() }}/customer/transport-delivered";
    });
    ...
    productDetail = (ref) => {
        ...
        $.ajax({
            url: "{{ url('customer/transpot/product-detail') }}",
            type: 'post',
            data: {
                _token: "{{ csrf_token() }}",
                ref: ref
            ...
            success: function(data) {
                ...
                $('#pd_title').html(`<h3>${data[0].ms_product_name}</h3> <h6>รหัสสินค้า: ${data[0].ms_product_code}</h6>`);
                $('#docs_no').html(`Order : ` + data[0].sal_confirmorder_hd_docuno);
                $('#price_sku').html(`฿ ${addComma(parseFloat(data[0].sal_salecommand_dt_price))}`);
                $('#qty_sku').html(`${addComma(parseFloat(data[0].deliverydocu_dt_qty))} ${data[0].ms_product_unit_name}`);
                $('#totalprice').html(`${addComma(parseFloat(data[0].sal_salecommand_dt_price) * parseFloat(data[0].deliverydocu_dt_qty))} บาท`);
