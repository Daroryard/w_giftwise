...
<div class="main-container">
    ...
    <div class="main">
                ...
                <div class="row">
                    <div class="col-sm-6">
                        <div class="page-title">
                            <h3 class="text-black">
                                @if (app()->getLocale() == 'th')
                                รายการรอยืนยันใบสั่งซื้อ ({{ $count_confirm }} รายการ)
                                @else
                                Confirm Order List ({{ $count_confirm }} items)                          
                                @endif
                            </h3>
                        </div>
                        <span class="product-tag link-confirm">
                            @if (app()->getLocale() == 'th')
                            <span class="badge badge-active">รอการยืนยัน</span>
                            @else
                            <span class="badge badge-active">Waiting for confirmation</span>
                            @endif
                        </span>
                        <span class="product-tag link-confirm-edit">
                            @if (app()->getLocale() == 'th')
        ...
        <div class="product-wrapper-grid">
            ...
            @foreach($production_confirm as $product)
...
<script>
    ...
    $(document).ready(function() {
        $('.menu-cus4').removeClass('option');
        $('.menu-cus4').addClass('option1');
    });


    $('.link-confirm').click(function() {
        location.href = "/{{ app()->getLocale() }}/customer/production-confirm";
    });
    $('.link-confirm-edit').click(function() {
        location.href = "/{{ app()->getLocale() }}/customer/production-confirm-wait";
    });
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#bt_action').html(`
                            ...
                            <button type="button" class="btn btn-solid" style="border-radius: 10px;" onclick="onFrim(${data[0].sal_confirmorder_dt_id})">ยืนยันใบสั่งซื้อ</button>
            `);
    ...
    $.ajax({
        url: "{{ url('customer/confrim-order') }}",
        type: 'post',
        data: {
            _token : "{{ csrf_token() }}",
            ref: ref
        ...
        success: function(data) {
            ...
            if(data.status == 'success') {
                Swal.fire({
                    ...
                    <br>
                    <small>สินค้าจะถูกนำไปผลิตต้นแบบ</small>`,
    ...
    $.ajax({
        url: "{{ url('/customer/generate-link') }}",
        type: 'post',
        data: {
            _token: "{{ csrf_token() }}",
            ref: ref
        ...
        success: function(data) {
            ...
            html: `
                ...
                inputValue: `{{ url('/customer/confirm-link') }}/${data.token}`,
                inputAttributes: {
                    readonly: true,
                    disabled: true,
                },
