            ...
            <div class="card">
                ...
                <div class="card-body">  
                    <div class="table-responsive">
                        <table id="table-quic" class="table align-middle table-nowrap table-hover">
                            <thead>
                                <tr>
                                    <th>ตำแหน่ง</th>
                                    <th>รูป</th>
                            ...
                            <tbody>
                                @foreach ($hd as $item)
                                    <tr>
                                        <td>{{$item->conf_productquick_type}}</td>
                                        <td>
                                            @if($item->conf_productquick_img)
                                                <a class="zoom-gallery" href="{{ asset($item->conf_productquick_img) }}"><img src="{{ asset($item->conf_productquick_img) }}" class="img-fluid" width="40"></a>
                                                @else
                                                <img src="https://placehold.co/600x400?text=No image" alt="" class="img-fluid" width="40">
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{ route('productquick.edit' , $item->conf_productquick_id) }}" class="btn btn-info"><i class="bx bx-edit"></i></a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
...
<script>
     $(document).ready(function() {
        var table =  $('#table-quic').DataTable({
            order: [[ 1, 'asc' ]],
            columnDefs: [
