            ...
            <div class="card">
                <div class="card-body border-bottom">
                    <div class="d-flex align-items-center">
                        <h5 class="mb-0 card-title flex-grow-1">ข้อมูลโปรเจค</h5>
                    ...
                    <div class="table-responsive">
                        <table id="table-project" class="table align-middle table-nowrap table-hover">
                            <thead>
                                <tr>
                                    <th>ชื่อโปรเจค</th>
                                    <th>จุดประสงค์ของงาน</th>
                                    <th>ธีมสินค้า</th>
                                    <th>สินค้าที่มองหา</th>
                                    <th>จำนวน</th>
                                    <th>ระยะเวลา</th>
                                    <th>รายละเอียด</th>
                                    <th>Name</th>
                            ...
                            <tbody>
                                @foreach ($hd as $item)
                                    <tr>
                                        <td>{{$item->docu_project_name}}</td>
                                        <td>{{$item->docu_project_objective}}</td>
                                        <td>{{$item->docu_project_themeproduct}}</td>
                                        <td>{{$item->docu_project_looking}}</td>
                                        <td>{{$item->docu_project_productqty}}</td>
                                        <td>{{$item->docu_project_productday}}</td>
                                        <td>{{$item->docu_project_need}}</td>
                                        <td>{{$item->docu_project_contactname}}</td>
                                        <td>{{$item->docu_project_email}}</td>
                                        <td>{{$item->docu_project_tel}}</td>
                                        <td>{{$item->docu_project_companyname}}</td>
                                        <td>{{$item->ms_customersub_code}}</td>
                                        <td>
                                            <a href="{{route('dueproject.edit',$item->docu_project_id)}}" class="btn btn-info"><i class="bx bx-edit"></i></a>
                                        </td>                                       
                                    </tr>
                                @endforeach
                            </tbody>
