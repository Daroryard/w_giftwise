    ...
    <div class="container-fluid">
        <div class="row">
            <div class="card">
                ...
                @endif
                <div class="card-body">  
                    <form class="outer-repeater needs-validation" novalidate action="{{ route('users-cust.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="col-12 col-md-6">
                            ...
                            <select class="form-control @error('email') is-invalid @enderror" autocomplete="off" id="email" name="email">
                                ...
                                @foreach ($cust as $item)
                                    <option value="{{$item->ms_customersub_email}}">{{$item->ms_customersub_code}}/{{$item->ms_customersub_name}}/{{$item->ms_customersub_email}}</option>
                                @endforeach
                            </select>
                            @error('email')
