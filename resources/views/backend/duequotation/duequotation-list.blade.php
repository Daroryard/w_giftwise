            ...
            <div class="card">
                <div class="card-body border-bottom">
                    <div class="d-flex align-items-center">
                        <h5 class="mb-0 card-title flex-grow-1">Project</h5>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="table-project" class="table align-middle table-nowrap table-hover">
                            <thead>
                                <tr>
                                ...
                                @foreach ($hd as $item)
                                    <tr>
                                        <td>
                                            ...
                                            <a class="zoom-gallery" href="{{ asset($item->ms_productsub_img1) }}"><img src="{{ asset($item->ms_productsub_img1) }}" class="img-fluid" width="100"></a>
                                            @else
                                            <img src="https://placehold.co/600x400?text=No image" alt="" class="img-fluid" width="100">
                                            @endif
                                        </td>
                                        <td>{{$item->sal_project_status_name}}</td>
                                        <td>{{$item->ms_product_code}}/{{$item->ms_product_name1}}</td>
                                        <td>{{$item->sal_quotation_dt_qty}}/{{$item->ms_product_unit_name}}</td>
                                        <td>{{$item->ms_typescreen_name}}</td>
                                        {{-- <td>{{$item->sal_quotation_hd_docuno}}</td>
                                        <td>{{$item->sal_project_hd_docuno}}</td>
                                        ...
                                        <td>
                                            ...
                                            @else
                                            <div class="progress">
                                                <div class="progress-bar" role="progressbar" style="width: {{$item->per}}%;" aria-valuenow="25" aria-valuemin="0" aria-valuemax="100">{{$item->per}}%</div>
                                            </div>
                                            @endif                       
                                        </td>
                                        <td>
                                            <a href="{{route('duequotation.edit',$item->sal_project_hd_docuno)}}" class="btn btn-info"><i class="bx bx-edit"></i></a>
                                            @if(Auth::user()->role == "superadmin")
                                            <a href="{{route('duequotation.show',$item->sal_project_hd_docuno)}}" class="btn btn-warning"><i class="bx bx-edit"></i></a>
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
